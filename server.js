...
function handleAction(world, action) {
  ...
  if (action === 'toggleCarry') {
    if (p.carrying) {
      ...
      const ground = itemAt(world, p.room);
      if (ground && dist(p.x, p.y, ground.x, ground.y) < TOUCH_DIST) {
        ground.carried = true;
        p.carrying = ground.type;
        world.lastEvent = `You pick up the ${labelFor(ground.type)}.`;
      }
    }
  }
}
...
function updateBat(world) {
  ...
  if (now >= bat.hopAt) {
    ...
    if (bat.carrying) {
      ...
      world.lastEvent = `A bat drops the ${labelFor(bat.carrying)} nearby!`;
      bat.carrying = null;
    } else {
      ...
      // from the player's hands, to keep this fair).
      const ground = itemAt(world, bat.room);
      if (ground && Math.random() < 0.35) {
        ground.carried = true;
...
function snapshot(world) {
  return {
    player: world.player,
    items: Object.fromEntries(Object.entries(world.items).map(([k, v]) => [k, { room: v.room, x: v.x, y: v.y, carried: v.carried }])),
