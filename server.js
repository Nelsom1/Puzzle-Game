...
const ROOMS = {
  ...
  r6: { name: 'Dead End Grove', exits: { W: 'r4', S: 'r9', E: { to: 'r_egg', lock: 'secretPassage', gateRoom: 'r6', hidden: true, custom: true } } },
  ...
  r13: {
    name: "Black Dragon's Pass",
    exits: {
      W: 'r12',
      N: { to: 'r16', lock: 'blackKey', gateRoom: 'r13' },
      S: { to: 'r_dotroom', lock: 'bridgeChamber', requiredItem: 'bridge', gateRoom: 'r13', hidden: true },
    },
  },
  ...
  r_dotroom: { name: 'Sealed Chamber', exits: { N: 'r13' } },
...
const ITEM_START = {
  ...
  dot: { room: 'r_dotroom', x: 700, y: 430 },
};
...
const START_ROOM = 'r0';
const WIN_ROOM = 'r14';
const BAT_EXCLUDED_ROOMS = new Set(['r14', 'r15', 'r16', 'r_dotroom', 'r_egg']);
const BAT_ROOM_POOL = Object.keys(ROOMS).filter(id => !BAT_EXCLUDED_ROOMS.has(id));

const SECRET_LINES = ['MOST OF THE YEAR,', 'PROBABLY AROUND 7 MONTHS.'];
const MESSAGE_ROOM = 'r_egg';
const SECRET_ROOMS = new Set(['r_dotroom', 'r_egg']);
// Each hidden passage the client shouldn't know about until it's unlocked.
const SECRET_GATES = [
  { hostRoom: 'r13', targetRoom: 'r_dotroom', lockFlag: 'bridgeChamber' },
  { hostRoom: 'r6', targetRoom: 'r_egg', lockFlag: 'secretPassage' },
];
...
// Static room metadata sent once to the client at connection time. Secret
// rooms are left out entirely until they're found.
const ROOM_META = {};
for (const id of Object.keys(ROOMS)) {
  if (SECRET_ROOMS.has(id)) continue;
...
function freshWorld() {
  ...
  return {
    ...
    unlocks: { yellowKey: false, whiteKey: false, blackKey: false, bridge: false, bridgeChamber: false, secretPassage: false },
    won: false,
    lost: false,
    lastEvent: null,
  };
...
const UNLOCK_MESSAGES = {
  yellowKey: 'The yellow castle gate creaks open!',
  whiteKey: 'The copper castle gate creaks open!',
  blackKey: 'The black castle gate creaks open!',
  bridge: 'The bridge settles into place -- the river crossing is open!',
  // bridgeChamber and secretPassage are intentionally left out here, so they
  // fall through to the vague generic message below -- those are the ones
  // meant to stay mysterious.
};

function updateUnlocks(world) {
  for (const room of Object.values(ROOMS)) {
    for (const exit of Object.values(room.exits)) {
      if (!exit.lock || exit.custom || world.unlocks[exit.lock]) continue;
      const gateRoom = exit.gateRoom;
      const requiredType = exit.requiredItem || exit.lock;
      const item = world.items[requiredType];
      const presentAsGround = item.room === gateRoom && !item.carried;
      const presentInHand = world.player.room === gateRoom && world.player.carrying === requiredType;
      if (presentAsGround || presentInHand) {
        world.unlocks[exit.lock] = true;
        world.lastEvent = UNLOCK_MESSAGES[exit.lock] || `The ${labelForLock(requiredType)} unlocks a gate somewhere nearby...`;
      }
    }
  }
}

// The final passage needs more than just the dot present -- echoing the
// original's requirement of a second, different object in the same room at
// the same time. The dot alone only gets you as far as the sealed chamber
// and back; revealing the passage needs a companion item left in Dead End
// Grove as well.
function updateSecretPassage(world) {
  if (world.unlocks.secretPassage) return;
  const dot = world.items.dot;
  const dotHere = dot.room === 'r6' && (!dot.carried || (world.player.room === 'r6' && world.player.carrying === 'dot'));
  if (!dotHere) return;
  const hasCompanion = Object.values(world.items).some(it => it.type !== 'dot' && it.room === 'r6' && !it.carried);
  if (hasCompanion) {
    world.unlocks.secretPassage = true;
    world.lastEvent = 'Something shifts nearby...';
  }
}

function labelForLock(lock) {
  return { yellowKey: 'yellow key', whiteKey: 'copper key', blackKey: 'black key', bridge: 'bridge', bridgeChamber: 'bridge' }[lock] || lock;
}
...
function snapshot(world) {
  return {
    ...
    secret: world.player.room === MESSAGE_ROOM ? { lines: SECRET_LINES } : null,
  };
}
...
wss.on('connection', (ws) => {
  ...
  const timer = setInterval(() => {
    ...
    updateUnlocks(world);
    updateSecretPassage(world);
    updateDragons(world);
    updateBat(world);
    checkWin(world);

    for (const gate of SECRET_GATES) {
      if (world.unlocks[gate.lockFlag] && !revealedGates.has(gate.targetRoom)) {
        revealedGates.add(gate.targetRoom);
        ws.send(JSON.stringify({
          type: 'reveal',
          room: gate.targetRoom,
          meta: buildRoomMeta(gate.targetRoom),
          patch: { room: gate.hostRoom, exits: buildRoomMeta(gate.hostRoom, true).exits },
        }));
      }
    }
